...
*Status:* Normative. *Scope:* This appendix defines the standard CE Suite capability
profiles — named tiers of implementation capability. A profile is a stable,
human-readable mapping from existing discoverable parameters to a certified capability
level. No new hardware is required; profiles are a naming convention over parameters
that are already readable from `ce_present` (Chapter 16) and the capability CSRs in
Chapter 13.
...
The `ce_present` CSR (0xFD0) and the capability CSRs (`cme_del_cap`,
`cme_bank_count`, etc.) let software probe the exact capabilities of a CE
implementation. For most software, this fine-grained probing is necessary and
correct.

For two groups of users, probing individual parameters is inconvenient:

[arabic]
. *Certification bodies.* Safety-certification standards (ISO 26262, DO-178C,
...
. *Firmware and OS integration code.* Boot code that configures CE can branch on a
...
as "unconstrained" may take any value.

==== B.3.1 CE-Embedded

*Target class.* Microcontrollers, deeply embedded bare-metal real-time systems,
RISC-V E-extension cores.
...
*Rationale.* `D = 0` means delegation is not possible — a single privileged mode
manages all ECIDs. This matches bare-metal embedded systems that have no concept of a
guest OS or hypervisor. CPE, MSE, and QoS are explicitly excluded to define a minimal
hardware target that can be implemented for a few hundred gates. `VMT = 0` removes
the requirement for vector/matrix register file save and restore, keeping implementation
cost minimal.
...
behavior: cache-partition isolation prevents best-effort tasks from evicting hot cache
lines belonging to real-time tasks. `D ≥ 1` allows the kernel (L0) to delegate banks
and CPE contracts to individual tasks (L1). Two NV banks are the minimum to execute a
fast-path context switch while keeping one bank holding the incoming context's state.
...
bounds cache interference, but DRAM access latency under contention is unbounded without
MSE arbitration. `D ≥ 2` supports a three-level hierarchy: host kernel (L0), a
hypervisor or partition manager (L1), and task-level ECIDs (L2). Four NV banks provide
...
realistic cloud workload. One VMT bank ensures at least one context can use
hardware-accelerated vector and matrix register save/restore.

'''''

=== B.4 Profile conformance check

Software determines profile conformance by reading the required CSRs and comparing
against the profile's constraint table. The CE-absent trap-handler probe is described
in Chapter 16 §4.1; the code below assumes CE is confirmed present.
...
CE-Embedded is in a separate branch (D = 0, CPE = 0), which conflicts with the other
three profiles. The capability space has the following structure:
...
advertise the profile name using the `ce,profile` string property on the relevant
hart node. This property is optional.
...
profile, firmware is incorrect; software must fall back to raw-CSR probing and log the
inconsistency — the same principle as `riscv,isa` vs. `ce_present` in Chapter 16 §5.

*No ISA string profile names.* Profile conformance is not expressed in the `riscv,isa`
ISA string. Profile names require CSR-value constraints (delegation depth, bank counts)
that the ISA string format cannot express. Profile conformance is determined from CSR
values or the `ce,profile` device-tree property only.
...
*Vendor profiles.* A vendor may define non-standard profiles using the prefix `V-`
followed by a short ASCII vendor identifier, a hyphen, and a descriptive name.
Example: `V-AcmeCorp-SecureRT`. The CE Suite specification does not define the
constraints or semantics of vendor profiles; software that encounters an unrecognized
profile name must fall back to raw-CSR probing.

*Composition.* Composing standard profiles is not defined in v1. Software requiring
a specific combination of a named profile and additional capabilities should use the
standard profile conformance check and then probe the additional capability separately.
Composition rules are deferred to a future version.
