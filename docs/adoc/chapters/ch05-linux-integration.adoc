...
____
*Note on offset 0.* Charter §3.2 mandates that `ecs_ptr` sits at
offset 0 of the `EC[e]` hardware entry, so that a single load
...
level it was given. (Charter §4.1: "`A child Group is delegated to a
child ECID and appears to the child as that child’s Group 0.`")
...
[width="100%",cols="25%,10%,40%,25%",options="header",]
|===
...
* *MSE* bounds the latency of cache misses to `(K{plus}1) × slot_size`
  (Chapter 9, §9.3).
