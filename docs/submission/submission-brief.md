...
transistor overhead per core (stratified by deployment class in Appendix C §C.4).
...
The 5–15% per-core transistor overhead (stratified by deployment class in
Appendix C §C.4) reflects three distinct deployment tiers. The figures below
are derived from `tools/ce-sizing-calculator.py` and calibrated against
concrete public baseline cores.

| Deployment class | CE SRAM total | vs in-order baseline¹ | vs large OoO baseline² |
|---|---|---|---|
| CE-MinimalRT (microcontroller / embedded RTOS) | 6.5 KB | 1.6–1.8% | 0.5% |
| CE-RT (multi-partition RT, mixed criticality) | 35.1 KB | 5.0–5.8% | 1.5% |
| CE-Full (server-class, nested virtualization) | 94.3 KB | 12.9–14.7% | 3.8% |

¹ In-order baseline: SiFive U84 (0.28 mm², 7 nm) / ARM Cortex-A55 (0.32 mm²,
7 nm); percentage range spans U84 (lower end) to A55 (higher end).  
² Large OoO baseline: SiFive P670-class with vector unit (~0.85 mm², 5 nm).  
Full breakdown by node, baseline, and component is in Appendix C §C.4 of the
specification, derived from `tools/ce-sizing-calculator.py`.

CE-Full at 12.9–14.7% against in-order baselines sits at the upper bound of
the headline range. Against large OoO baselines where CE-Full is the natural
deployment, the per-core overhead is 3.8%. The 5–15% range accurately
characterises the CE-RT tier across realistic baseline cores.

The per-core cost breaks down by component. CPE tag bits are charged to the
cache arrays, not to the core CE block; MSE and QoS arbiter logic is a
one-time per-chip cost amortised across all cores on the SoC.

| Component | Per-core | Per-chip |
|---|---|---|
| CME Bank SRAM (NV + VMT banks) | Dominant; scales with bank count | — |
| EC[e] table hot set | Small fixed (~0.5–4 KB) | — |
| Bank tags (owner ECID, dirty, lock) | Tiny (~16–272 B) | — |
| S/R staging banks | ~2–10 KB | — |
| Copy engine + decoders | Small fixed logic | — |
| CPE tag bits (in cache, not core) | Adds to cache tag SRAM | — |
| MSE arbiter (memory controller) | — | One-time per chip |
| QoS arbiter (NoC / DMA) | — | One-time per chip |
| Instruction decode | Negligible | — |
| Discovery CSRs | Negligible | — |

MSE and QoS arbiter costs amortise well on many-core SoCs: on a 16-core
CE-RT deployment, the per-core arbiter amortisation adds less than 0.3%.

The CE-RT overhead is in the same range as adding a hardware double-precision
FPU to a comparable core; the CE-Full overhead is comparable to adding both
an FPU and a hardware vector unit, depending on deployment class. CE Suite
provides in return: provable WCET, 1–9 cycle context switches, and a
