...
* **Syntax**: `ec.ob rd, rs1, rs2`
  * `rd`: 0 on success; `CME_ERR_INVALID_ECID` if the target ECID is not allocated or
    ...
    caller is not an authorized ancestor; `CME_ERR_ALREADY_SEALED` (6) if the
    target bank is sealed — use `ec.ov` to unseal before restoring.
  * `rs1`: Target ECID and expected generation, packed in a single register:
    * `rs1[15:0]` — ECID slot index (the target context).
    * `rs1[23:16]` — Expected generation counter. Hardware compares this against
      `EC[target].generation`; a mismatch returns `CME_ERR_INVALID_ECID` without
      modifying any state. This detects stale references — slots that were freed
      and reallocated since the caller last obtained the ECID (ch00 §0.2).
    * `rs1[63:24]` — Must be zero; non-zero returns `CME_ERR_ILLEGAL_FIELD`.

    **Obtaining the generation:** after `ec.ir` returns the new ECID in `rd`,
    the OS reads `EC[rd].generation` from the EC array in memory
    (at `cme_ec_table_base + rd × stride`, offset 8 from `ecs_ptr`).
    The OS stores the `(ecid, generation)` pair and passes the packed value
    in rs1 for all subsequent `ec.ob` calls targeting that context.
...
These operations seal and unseal banks under hardware-managed encryption using
the key material in `cme_seal_key` (Chapter 13 §3.9). Both instructions are
M-mode only.

**What is normative here:** the instruction-level behaviour — what constitutes a
sealed bank, how sealing interacts with other CME instructions, and the error
conditions. **What is deferred to a future revision:** the encryption algorithm
(implementation-defined in v1.0), key derivation, key rotation, attestation, and
whether a sealed bank's ciphertext is portable across harts or implementations.

### Sealed bank state

A bank is either *unsealed* (normal) or *sealed* (encrypted). Hardware tracks
this per-bank. The transitions are:

- Any bank starts unsealed.
- `ec.iv` transitions an unsealed bank to sealed.
- `ec.ov` transitions a sealed bank back to unsealed on successful authentication.
- `ec.ob` refuses to restore a sealed bank — returns `CME_ERR_ALREADY_SEALED` (6).
  Unseal with `ec.ov` before restoring.
- `ec.im`/`ec.om` may spill and fill sealed banks as opaque ciphertext; sealed
  state is preserved across the spill/fill cycle.
...
* **Syntax**: `ec.iv rd, rs1, rs2`
  ...
  * `rs2`: Register mask — which groups to seal (same encoding as `ec.ib`).
* **Semantics**: Hardware encrypts the register groups in `rs2` in the bank owned
  by ECID `rs1` using the key material in `cme_seal_key`. The bank is marked
  sealed. The encryption algorithm is implementation-defined.
* **Privilege**: M-mode only.
* **Error conditions**: `CME_ERR_INVALID_ECID` (1), `CME_ERR_ALREADY_SEALED` (6).
  See Chapter 15 §15.5.1.
...
* **Syntax**: `ec.ov rd, rs1, rs2`
  * `rd`: 0 on success; `CME_ERR_NOT_SEALED` (7) on authentication failure.
  * `rs1`: Target ECID whose bank is to be unsealed.
  * `rs2`: Register mask.
* **Semantics**: Hardware decrypts the sealed bank for ECID `rs1` using the key
  material in `cme_seal_key`. On successful authentication the bank is marked
  unsealed and the groups in `rs2` become accessible. On failure (wrong key,
  tampered ciphertext, or bank not sealed) returns `CME_ERR_NOT_SEALED` without
  modifying the bank.
* **Privilege**: M-mode only.
* **Note**: Key derivation, attestation, and rotation are deferred to a future
  revision. Implementations must ensure `cme_seal_key` holds consistent material
  between the sealing and unsealing calls for a given bank.
...
| `0000001`   | `ec.ob`  | result        | ECID + generation¹ | mask register      |
...
¹ `ec.ob` rs1 packs `(generation[23:16] | ecid[15:0])`. Bits [63:24] must be zero.
  See §3.1 for details.
...
```
  ...
  ─── Step 2: ec.ob x0, B_ref, mask ─────────────────────────────────
    B_ref = (gen_B << 16) | ecid_B   (packed register value)
    EC[ecid_B].generation checked against gen_B  (stale → INVALID_ECID)
    EC[ecid_B].bank_ref  ──→  Bank[B] located
    Bank[B]              ──→  Active reg file  (state restored from SRAM)
    current_ecid         ──→  ecid_B           (updated on commit)
