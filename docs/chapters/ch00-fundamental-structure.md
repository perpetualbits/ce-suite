...
independent enable bit; all reset to 0. When an extension's enable bit is 0,
all its instructions trap as illegal and all its CSRs read as 0. Extensions
may be enabled independently. Even when CE is enabled, any privilege level may
choose to ignore it and run a conventional kernel or userspace. This opt-in
property applies uniformly to all five extensions: CME, CPE, MSE, QoS, and
the ECID substrate.
...
If bit 6 (SATP) is set in an `ec.ob` mask and the restored SATP differs from
the current SATP, hardware performs a TLB invalidation atomically with the
restore; implementations may skip the invalidation when SATP is unchanged. Full
semantics are in Chapter 3 §3.1 (TLB Invalidation on SATP Restore); normative
authority is the CE Suite Project Instructions §6.8.
...
- **CME (banks).** Each delegation level sees its own Group as "Group 0." The
  parent's GroupID is invisible to the child. See §0.5 "Child renaming." Hardware
  ownership enforcement uses up-pointers (Banks point to their owning Group); this
  allows each level to operate on its delegated banks as if they were the only banks
  in its world.

- **MSE (memory bandwidth).** Each delegation level sees its bandwidth allocation on
  the same 0–255 local scale, where 256 represents 100% of *its own slice*. The
  actual fraction of total system bandwidth depends on the level's depth (each
  delegation reduces the slice), but the running software is unaware. The hardware
  applies Formula 2 of Chapter 9 §9.4.6 to convert internal stored-global values to
  local-view CSR readbacks at the running EC's level. See charter §4.5.0.
