...
This chapter is the normative reference for the semantics of Groups, Banks,
Contracts, and delegation in the CE Suite. It specifies:

- The Group model: what a Group is, how it is identified, and how ownership
  is enforced.
...
6-bit group number, and no instruction that creates a Group independently of
an ECID. When an ECID is allocated, its Group comes into existence with it.
When the ECID is destroyed, the Group is destroyed with it.
...
ownership, hardware reads that field and compares it to `current_ecid` —
...
To ask "what does ECID `e` own?" you search the Bank and Contract tables for
entries whose owner field equals `e`. To ask "does ECID `e` own Bank `b`?"
you read `b.owner` and compare. The latter question is the one hardware asks
at runtime, and it is answered in one instruction.

This is the **reversal trick**: ownership flows from resource to Group, not
...
child's own Group. The child cannot observe the host-level GroupID (its ECID
number is opaque to software running as that EC — see Chapter 1 §1.3), and
cannot infer the ECID numbers or Group IDs of its parent, siblings, or
ancestors. Each delegation level renumbers its world to start at zero.

From the **hardware's perspective**, every Bank and Contract in the system
has an explicit ECID owner number. The hierarchy is tracked in the radix tree
(Appendix A) and in `EC[e].parent_ecid` (Chapter 0 §0.2). There is no
...
| **Owned**  | Assigned to an ECID's Group. The Bank's owner field equals that ECID. |
| **Active** | Owned and currently in use by the running context (`current_ecid`). |
| **Sealed** | Owned and encrypted under hardware key management (`ec.iv`). Contents inaccessible until unsealed by the owning EC. |
...
RV64, 512 B on RV32. Layout in Chapter 0 §0.4.
...
significantly larger than non-VMT banks and are typically fewer in number.

Non-VMT and VMT banks are distinct pools. `ec.ig` and `ec.og` on a VMT
bank require that the target ECID has VMT delegation rights (specified in
...
- **Owner ECID**: the ECID number of the Group currently holding this Contract.
...
same up-pointer pattern as for Banks. Ownership can be transferred but not
duplicated.
...
atomically, or fails and leaves all state unchanged. There is no partial
success. This is necessary to prevent double-counting: without atomicity,
...
1. All child Contracts of the dissolved Contracts are themselves dissolved
   recursively.
2. The allocations return to the dissolved Contracts' parent Contracts,
   making them available for re-splitting.
3. If the dissolved Contract was a root Contract, its allocation returns
...
| Condition | Meaning                                                            |
|-----------|--------------------------------------------------------------------|
| `L < D`   | This ECID may create child ECIDs and delegate Banks and Contracts. |
...
Contract trees are bounded by the same D. A Contract created at level `L`
can be split into children only if `L < D`. This ensures the Contract
...
1. The child ECID's Group owns the delegated resources. The parent's Group
   does not.
2. The parent cannot save to, restore from, or modify a delegated Bank.
   The bank's owner field no longer matches the parent's ECID.
3. The parent cannot use a delegated Contract to request bandwidth or
   latency guarantees. The Contract's owner field no longer matches the
   parent's ECID.
4. The child may further delegate (if `L < D`) or use resources for itself
   (if `L = D`).
5. The parent retains its own non-delegated Banks and Contracts.
...
kernel, other than updating its own software data structures (ECS objects,
radix-tree bookkeeping in RAM).
...
2. **Unique Contract ownership.** No Contract may have more than one owner
   ECID.
3. **Contract sub-sum.** For any Contract `C` with children `C_1..C_n`:
   sum of children's allocations ≤ `C`'s allocation.
4. **Delegation depth.** No ECID may have delegation level `L > D`. No
...
construct visible to the hardware. The hardware sees `EC[e]` — an array
indexed by ECID number with a fixed-stride layout. The radix tree populates
and de-populates `EC[e]` entries; hardware queries `EC[e]` directly.

The radix tree provides two properties that the hardware `EC[e]` array
alone does not:

1. **Prefix ownership.** A tenant owns an entire subtree of ECID numbers.
   Allocations within a prefix do not require cross-tenant coordination.
2. **Fast forced revocation.** Destroying a subtree rooted at ECID `e`
   traverses the radix tree in O(log N) steps, collecting all descendant
   ECIDs to be freed.

The detailed radix-tree data structure and the allocation, delegation, and
