...
### 1.1 `ce_ctrl` — CE extension enable (0x7D0)

The `ce_ctrl` CSR controls which CE Suite extensions are active on this hart.
It is M-mode read/write and resets to 0 (all extensions disabled). Firmware
explicitly enables whichever extensions are required.

| Bit | Field | Access | Reset | Description |
|-----|-------|--------|-------|-------------|
| 0 | `CME_EN` | WARL | 0 | 1 = CME enabled; 0 = CME instructions trap, CME CSRs read 0 |
| 1 | `CPE_EN` | WARL | 0 | 1 = CPE enabled; 0 = CPE instructions trap, CPE CSRs read 0 |
| 2 | `MSE_EN` | WARL | 0 | 1 = MSE enabled; 0 = MSE instructions trap, MSE CSRs read 0 |
| 3 | `QOS_EN` | WARL | 0 | 1 = QoS enabled; 0 = QoS instructions trap, QoS CSRs read 0 |
| XLEN−1:4 | — | WIRI | 0 | Reserved |

**WARL range.** If hardware for an extension is absent, the corresponding bit
is RO 0; writing 1 has no effect. Software checks `ce_present` (0xFD0,
Chapter 16) to determine which extensions are implemented before enabling them.

**Independence.** Extensions may be enabled and disabled independently in any
order. There is no required enable sequence.

**Effect.** When an extension's enable bit is 0, all instructions for that
extension trap as illegal instruction and all CSRs for that extension read as 0,
regardless of their individual addresses or access types. This is the mechanism
behind the "CE disabled" behavior described in §2.
...
| 0x7D0 | `ce_ctrl` | CE substrate | WARL | Per-hart |
